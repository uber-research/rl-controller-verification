...
load("@brezel//rules/pip:local_install.bzl", "pip_local_install")
pip_local_install(
    name = "gym_envs",
    manifest = "//:gym-quadcopter/MANIFEST.in",
)

load("@//third_party/rtamt:deps.bzl", "rtamt_deps")
rtamt_deps()
load("@//third_party/rtamt:pip_deps.bzl", "rtamt_pip_deps")
rtamt_pip_deps()
